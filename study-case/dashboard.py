...
# Memastikan order_date and delivery_date bertipe date time dan mengurutkan untuk filter
...
ax[1].yaxis.set_label_position("right")
ax[1].yaxis.tick_right()
ax[1].set_title("Wrost Performing Product", loc="center", fontsize=50)
ax[1].tick_params(axis='y', labelsize=35)
ax[1].tick_params(axis='x', labelsize=30)
...
with col2:
    avg_recency = round(rfm_df.frequency.mean(), 2)
    st.metric("Average Frequency", value=avg_recency)
